...
class MetaCorgiPerkPackage
  ...
  #   item = name
  #
  #   info = @shopping_list.send("get_#{item}_info", @package_id)
  #   happiness = @shopping_list.send("get_#{item}_happiness", @package_id)
  #   result = "#{item.capitalize}: #{info}: #{happiness} licks"
  #   happiness > 30 ? "* #{result}" : result
  # end
